package com.gitile.base.mybatis.mapper;
...
public class BuildMapper {
	...
	public void run(String configPath) {
		...
		if(project!=null) {
			...
			for (Table table:tables) {
				String tableName = table.getBean();
		...
		try {
			...
			FileUtils.buildTemplateFile(rootMap, StringUtils.buildResourcesPathMaven(mybatisPath) + File.separator + "mybatis-config.xml", template);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void tableToMapper(Module project, Table table) {
		...
		try {
			...
			FileUtils.buildTemplateFile(rootMap, StringUtils.buildResourcesPathMaven(mapperPath) + File.separator + modelName + "Mapper.xml", template);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
